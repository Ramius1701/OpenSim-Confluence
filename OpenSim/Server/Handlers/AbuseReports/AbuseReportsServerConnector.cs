...
namespace OpenSim.Server.Handlers.AbuseReports
{
    public class AbuseReportsServiceConnector : ServiceConnector
    {
                ...
                base(config, server, configName)
        {
            ...
            // Separate secret gating report *retrieval* (an admin-tool
            // concern) from whatever auth gates report *submission*.
            string adminSecret = serverConfig.GetString("AdminSecret", String.Empty);

            server.AddStreamHandler(new AbuseReportsServerPostHandler(m_AbuseReportsService, auth, adminSecret));
        }
    }
}
